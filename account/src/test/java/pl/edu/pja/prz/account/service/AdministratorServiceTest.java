...
class AdministratorServiceTest {
	...
	@Mock private AdministratorAccountFactoryImpl administratorAccountFactory;
	@Mock private BoroughRepository boroughRepository;
	...
	void createGuardianAccount() {
		...
		when(administratorAccountFactory.createStandardAccount(
						any(Address.class),any(FullName.class),any(Phone.class),any(Password.class),any(),any()
				)).thenReturn(guardian);
	...
	void createEmployeeAccount() {
		//given
		var employee = new AdministratorAccountFactoryImpl().createTeacher(address,fullName,phone,password,email);
	...
	void createAdministratorAccount() {
		...
		var administrator = new AdministratorAccountFactoryImpl().createAdministrator(address,fullName,phone,password,email);

		//when
		when(passwordManager.generateEncodeRandomPassword()).thenReturn(password.getPassword());
		when(administratorAccountFactory.createAdministrator(
				any(Address.class),any(FullName.class),any(Phone.class),any(Password.class),any()
